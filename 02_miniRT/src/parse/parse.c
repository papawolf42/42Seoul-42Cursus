...
/*   Updated: 2021/05/13 22:02:17 by gunkim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_parse_rt(t_ctrl *ctrl, char *rt_file)
{
	...
	ctrl->scene->canv.width = 600;
	ctrl->scene->canv.height = 600;
	ctrl->scene->canv.aspect_ratio = (double)ctrl->scene->canv.width / (double)ctrl->scene->canv.height;
	ft_object_list_init(ctrl->scene);
	ft_object_list(cam, ft_camera(V_POINT(0, 1, 0), V_SET(0, -1, 1), 60, 2.0, 1.0, ctrl->scene->canv.aspect_ratio), ctrl->scene->camera_list);
