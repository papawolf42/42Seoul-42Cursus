...
/*   Updated: 2021/05/02 04:19:17 by gunkim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_parse_rt(t_ctrl *ctrl, char *rt_file)
{
	...
	ctrl->scene->canv.aspect_ratio = (double)1600 / (double)900;
	...
	ft_object_list(cam, ft_camera(V_SET(0, 0, 0), 2.0, 1.0, ctrl->scene->canv.aspect_ratio), ctrl->scene->camera_list);
	ft_object_list(sp, ft_sphere(V_POINT(0, -0.5, -2), 0.5, V_COLOR(0.616, 0.710, 0.800)), ctrl->scene->object_list);
	ft_object_list(sp, ft_sphere(V_POINT(1, -0.5, -2), 0.5, V_COLOR(0.6, 0.714, 0.675)), ctrl->scene->object_list);
	...
	// ft_object_list(l, ft_light(V_SET(0, 1.5, 1), V_COLOR(1, 1, 1)), ctrl->scene->light_list);
	ft_object_list(l, ft_light(V_SET(-0.4, -0.5, -0.5), V_COLOR(2, 2, 2)), ctrl->scene->light_list);
	// ft_object_list(l, ft_light(V_SET(0.5, -0.5, -1), V_COLOR(1, 1, 1)), ctrl->scene->light_list);
	// ft_object_list(sp, ft_sphere(V_SET(0, 0, -2), 0.5, V_COLOR(0.5, 0.3, 0.3)), ctrl->scene->object_list);
	// ft_object_list(sp, ft_sphere(V_SET(0, -1, -2), 0.5, V_COLOR(0.2, 0.2, 0.2)), ctrl->scene->object_list);
	// ft_object_list(l, ft_light(V_SET(0, 2.2, -2.8), V_COLOR(0.6, 1, 1)), ctrl->scene->light_list);
	// ft_object_list(l, ft_light(V_SET(1, 1.5, 0), V_COLOR(16, 16, 16)), ctrl->scene->light_list);
