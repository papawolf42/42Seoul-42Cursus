...
void		ft_parse_rt(t_ctrl *ctrl, char *rt_file)
{
	...
	// 	while(x < 19)
	// 	{
	// 		ft_object_list(sq, ft_square(V_POINT(-4.5 + 0.5 * x, -4.5 + 0.5 * y, -5), V_SET(-0.9 + 0.1 * x, -0.9 + 0.1 * y, 1), 0.5, V_COLOR(0.5, 0.5, 0.5)), ctrl->scene->object_list);
	// 		x += 1;
	// 	}
	// 	y += 1;
	// }
	...
	// ft_object_list(l, ft_light(V_SET(0, 0, 0), V_COLOR(0.2, 0.2, 0.2)), ctrl->scene->light_list);
	/* to avoid warning */
	rt_file = NUL;
	/* to avoid warning */
}