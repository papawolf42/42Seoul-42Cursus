...
/*   Updated: 2021/05/24 11:57:39 by gunkim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "struct.h"

t_point3	ft_screen(t_ctrl *c, double fov, double x, double y)
{
	t_point3	screen;

	screen.x = (2 * ((x + 0.5) / (double)c->scene->canv.width) - 1) * fov
				* c->scene->canv.aspect_ratio;
	screen.y = (-2 * ((y + 0.5) / (double)c->scene->canv.height) + 1) * fov;
	screen.z = -1;
	return (screen);
}

t_point3	ft_screen_fisheye(t_ctrl *c, double fov, double x, double y)
{
	t_point3	screen;

	screen.x = (2 * ((x + 0.5) / (double)c->scene->canv.width) - 1);
	screen.x *= INDEX_FISH_EYE;
	screen.y = (-2 * ((y + 0.5) / (double)c->scene->canv.height) + 1);
